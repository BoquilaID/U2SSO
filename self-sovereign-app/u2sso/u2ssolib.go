...
func RegistrationProof(index int, currentm int, currentN int, serviceName []byte, challenge []byte, mskBytes []byte, idList [][]byte) (string, []byte, bool) {
	...
	if int(C.secp256k1_boquila_derive_spk(ctx, &rctx, &spk, sskPtr)) == 0 {
	...
	if int(C.secp256k1_boquila_auth_prove(ctx, &rctx, proofnewPtr, mskPtr, rPtr, namePtr, name_len, &spk, chalPtr)) == 0 {
		fmt.Println("auth prove failed")
		return "", nil, false
	}
	if int(C.secp256k1_boquila_auth_verify(ctx, &rctx, proofnewPtr, &spk, chalPtr)) == 0 {
...
func AuthVerify(proofHex string, serviceName []byte, challenge []byte, spkBytes []byte) bool {
	...
	if int(C.secp256k1_boquila_auth_verify(ctx, &rctx, proofPtr, &spk, chalPtr)) == 0 {
		fmt.Println("auth verify failed")
		return false
	}
	return true

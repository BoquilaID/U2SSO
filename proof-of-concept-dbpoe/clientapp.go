...
func main() {
	...
	if commandFlag == "create" {
		...
		topicList, err := instance.GetAllServices(nil)
		if err != nil {
			fmt.Println("Could not get topic list from ", contractFlag)
			return
		}
		topicListSize := len(topicList)
		idBytes := u2sso.CreateID(mskBytes, topicList, topicListSize)
	...
	if commandFlag == "register" {
		...
		topicIndex := u2sso.GetServiceIndexfromContract(instance, serviceName)
		proofHex, spkBytes, nullifier, val := u2sso.RegistrationProof(int(index), currentm, int(idsize.Int64()), serviceName, challenge, mskBytes, IdList, topicList, topicListSize, int(topicIndex))
		...
		fmt.Println("nullifier hex format: ", hex.EncodeToString(nullifier))
	}
	...
	if commandFlag == "auth" {
		...
		topicListSize := len(topicList)
		proofAuthHex, val := u2sso.AuthProof(serviceName, challenge, mskBytes, topicList, topicListSize)
